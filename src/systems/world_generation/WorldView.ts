...
export class WorldView {
  ...
  public create(): void {
    ...
    this.worldModel.forEachTile((x, y, tile) => {
      let pixelPosition: Phaser.Math.Vector2 =
        this.hexGrid.convertGridHexToPixelHex(new Phaser.Math.Vector2(x, y));

      let tileSpriteKey: string = tile.terrainData.name;

      let terrainSprite: Phaser.GameObjects.Image = this.scene.add.image(
        pixelPosition.x,
        pixelPosition.y,
        tileSpriteKey
      );
      terrainSprite.setScale(
        this.hexGrid.hexScale,
        this.hexGrid.hexScale * 1.065
      ); // Magic number 1.065 is because the hex sprites are slightly squashed
    ...
    this.worldModel.forEachTile((x, y, tile) => {
      if (tile.structureData === undefined) return;
      ...
      if (
        tile.structureData.alt_path !== undefined &&
        this.randomGenerator.between(0, 1) == 1
      ) {
        structureSpriteKey += '_alt';
      }

      let structureSprite: Phaser.GameObjects.Image = this.scene.add.image(
        pixelPosition.x,
        pixelPosition.y,
        structureSpriteKey
      );
      structureSprite.setScale(
        this.hexGrid.hexScale * tile.structureData.sprite_scale,
        this.hexGrid.hexScale * tile.structureData.sprite_scale
      );
