...
export class RegionOutline {
    ...
    constructor(scene: Phaser.Scene, hexGrid: HexagonGrid, hexes: Phaser.Math.Vector2[], colour: number, thickness: number, draw: boolean = true) {
    ...
    public setHexes(newHexes: Phaser.Math.Vector2[], redraw: boolean = true): void {
    ...
    public draw(): void {
        ...
        for (let hex of this.hexes) {
            ...
            let neighboringHexes: Phaser.Math.Vector2[] = this.hexGrid.getNeighbouringHexes(hex);

            for (let neighbourHex of neighboringHexes) {

                let adjacentHexAlsoInRegion = this.hexes.some(item => item.x == neighbourHex.x && item.y == neighbourHex.y);

                if (adjacentHexAlsoInRegion == false) {

                    // Find the two common vertices and draw a line
                    let primaryHexCorners: Phaser.Math.Vector2[] = this.hexGrid.getPixelHexCorners(hex);
